...
class PreferenceUtils(private val context: Context) {
    ...
    val aspectRatio: Flow<Int> = context.dataStore.data
        .map { prefs -> prefs[KEY_ASPECT_RATIO] ?: ASPECT_FIT }

    val subtitleEnabled: Flow<Boolean> = context.dataStore.data
        .map { prefs -> prefs[KEY_SUBTITLE_ENABLED] ?: true }

    val sortOrder: Flow<Int> = context.dataStore.data
        .map { prefs -> prefs[KEY_SORT_ORDER] ?: SORT_BY_DATE }

    val viewMode: Flow<Int> = context.dataStore.data
        .map { prefs -> prefs[KEY_VIEW_MODE] ?: VIEW_LIST }

    val rememberPosition: Flow<Boolean> = context.dataStore.data
        .map { prefs -> prefs[KEY_REMEMBER_POSITION] ?: true }

    val hardwareDecode: Flow<Boolean> = context.dataStore.data
        .map { prefs -> prefs[KEY_HARDWARE_DECODE] ?: true }

    val pipEnabled: Flow<Boolean> = context.dataStore.data
        .map { prefs -> prefs[KEY_PIP_ENABLED] ?: true }
    ...
    suspend fun setSubtitleEnabled(enabled: Boolean) {
        context.dataStore.edit { it[KEY_SUBTITLE_ENABLED] = enabled }
    }
    ...
    suspend fun setHardwareDecode(enabled: Boolean) {
        context.dataStore.edit { it[KEY_HARDWARE_DECODE] = enabled }
    }

    suspend fun setPipEnabled(enabled: Boolean) {
        context.dataStore.edit { it[KEY_PIP_ENABLED] = enabled }
    }
}
