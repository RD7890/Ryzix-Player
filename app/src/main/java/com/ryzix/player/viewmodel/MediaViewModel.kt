...
class MediaViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun saveWatchHistory(path: String, title: String, duration: Long, position: Long) {
        viewModelScope.launch(Dispatchers.IO) {
            dao.insertOrUpdate(WatchHistory(
                videoPath = path, title = title, duration = duration,
                lastPosition = position, watchedAt = System.currentTimeMillis()
            ))
        }
    }
