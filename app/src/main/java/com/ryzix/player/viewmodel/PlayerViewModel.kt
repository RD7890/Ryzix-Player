...
class PlayerViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun stopPositionTracking() { positionTrackJob?.cancel() }

    fun toggleLock() { _isLocked.value = !(_isLocked.value ?: false) }
    ...
    fun showControls() { _showControls.value = true; scheduleHideControls() }
    fun hideControls() { _showControls.value = false; hideControlsJob?.cancel() }
    ...
    fun savePosition(position: Long) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            dao.insertOrUpdate(WatchHistory(
                videoPath = currentVideoPath, title = currentVideoTitle,
                duration = currentDuration, lastPosition = position,
